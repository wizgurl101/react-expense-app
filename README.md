# Expense Tracking Web Application

<p align="center">
<img src="expenseTracker1.jpg" width="80%">
</p>

## Description

This web application let user to add the number expenses they have in a month. The application will
display a chart of all expenses of a given calendar year. Application does not presist the user's
data, therefore, it will be lost when the user closed the it.

## About the project

This project was one I coded while following along Academind's React the Complete Guide course.

## What have I learnt from this project

React basics such as it component approach to building UI, how to pass data between components by the props system,
JSX, stateful List and two-way binding.

<p align="center">
<img src="expenseTracker2.jpg" width="80%">
</p>
<p align="center">
<img src="expenseTracker3.jpg" width="80%">
</p>
